%% Compute Event related spectral perturbations for single trials for single ICs in RF
...
% - save ERSP matrixes for right frontal cluster ICs FOR EACH TRIAL

clear all
clc

[folder, name, ext] = fileparts(which('computeERSP_st.m'));
cd([folder '/'])
home;
current_folder = pwd;
...
for fa = 1:length(Cls_SUBJECT)
    ...
    save([mypath 'ERSP_RF_TF_st_' num2str(fa) '.mat'],...
    'tfdataGo',...
    'tfdataFail',...
    'tfdataSucc',...
    'tfdataStand',...
